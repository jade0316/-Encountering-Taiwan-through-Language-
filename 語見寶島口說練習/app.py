...
with st.sidebar:
    st.header("⚙️ 設定")
    # 這裡改為從 Streamlit Secrets 讀取 Key (如果有的話)
    # 這樣老師就不用每次輸入，學生也不會看到 Key
...
with col2:
    ...
    # 這裡使用 audio_recorder 元件，它會處理網頁端的錄音
    ...
    # 一旦 audio_bytes 有資料，代表錄音結束
    if audio_bytes:
        st.audio(audio_bytes, format="audio/wav") # 讓學生可以回放聽聽看
        ...
        with st.spinner("正在分析您的發音..."):
            ...
            if api_mode == "真實模式 (Azure API)":
                if not azure_key or not azure_region:
                    st.error("⚠️ 尚未設定 Azure API Key，請聯絡老師或使用演示模式。")
                else:
...
if 'result' in st.session_state:
    ...
    st.write("")
    if accuracy < 80:
        st.warning("⚠️ **發音準確度**待加強：請留意聲調。")
    elif fluency < 75:
        st.warning("⚠️ **流暢度**待加強：請唸得更連貫。")
    elif total >= 90:
        st.balloons()
